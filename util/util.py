...
def load_data(data_folder, fnames=['train_FD001', 'train_FD002', 'train_FD003', 'train_FD004']):
    # Read the CSV files
...
def split_train_test_machines(data, tr_ratio=0.75, seed=None):
    # Shuffle the machine numbers
    np.random.seed(seed)
    machines = data.machine.unique()
    np.random.shuffle(machines)
    # Split the machines in two groups
    sep = int(tr_ratio * len(machines))
    tr_mcn = machines[:sep]
    ts_mcn = machines[sep:]
    # Split the dataset in two
    tr, ts = partition_by_machine(data, tr_mcn)
    return tr, ts
...
def rescale_CMAPSS(tr, ts, ts2=None):
    ...
    # Rescale the additional set, if provided
    if ts2 is not None:
        ts2_s = ts2.copy()
        ts2_s[dt_in] = (ts2_s[dt_in] - trmean) / trstd
    tr_s = tr.copy()
    ...
    if ts2 is None:
        res = [tr_s, ts_s, params]
    else:
        res = [tr_s, ts_s, ts2_s, params]
    return res
...
def plot_pred_scatter(y_pred, y_true, figsize=None,
    ...
    plt.show()
    if print_r2_score:
        print(f'R2 score: {r2_score(y_true, y_pred):.3}')
...
def plot_confusion_matrix(y_true, y_pred, figsize=None):
    plt.figure(figsize=figsize)
...
def plot_signal(signal, labels=None,
        figsize=None, autoclose=True, s=4):
    if autoclose:
        plt.close('all')
    plt.figure(figsize=figsize)
...
def plot_bars(data, figsize=None, autoclose=True, tick_gap=1):
